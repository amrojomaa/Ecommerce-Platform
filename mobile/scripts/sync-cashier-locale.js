...
const shouldSyncKey = (key) =>
  key.startsWith('ui.pages.cashier.') ||
  key === 'ui.context.confirmContext.cancel_c50fab1ce7' ||
  key === 'ui.components.commentSection.close_2bcc546846';
...
for (const lang of LANGS) {
  ...
  const cashierKeys = Object.keys(web).filter(shouldSyncKey);
  const merged = { ...mobile };
  cashierKeys.forEach((k) => {
    merged[k] = web[k];
  });
  ...
  console.log(`${lang}: added/updated ${cashierKeys.length} cashier keys`);
}
