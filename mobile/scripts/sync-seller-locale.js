...
const shouldSyncKey = (key) => key.startsWith('ui.pages.seller.');
...
for (const lang of LANGS) {
  ...
  const sellerKeys = Object.keys(web).filter(shouldSyncKey);
  const merged = { ...mobile };
  sellerKeys.forEach((k) => {
    merged[k] = web[k];
  });
  ...
  console.log(`${lang}: added/updated ${sellerKeys.length} seller keys`);
}
