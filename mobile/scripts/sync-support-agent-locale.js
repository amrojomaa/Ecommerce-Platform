...
const shouldSyncKey = (key) =>
  key.startsWith('ui.pages.support_agent.') ||
  key.startsWith('ui.components.supportTicketChatModal.');
...
for (const lang of LANGS) {
  ...
  const supportKeys = Object.keys(web).filter(shouldSyncKey);
  const merged = { ...mobile };
  ...
  console.log(`${lang}: added/updated ${supportKeys.length} support_agent keys`);
}
